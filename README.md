# 📘 CEDT 2110473 Fault Tolerant Computing - Assignment Repository

This repository contains my cedt fault tolerant computing homework solutions, written as **Jupyter notebooks** (Python) and, where a written proof or derivation is needed, in **LaTeX**.
...
.
├── homework-01/
│   ├── homework-01.tex          # LaTeX write-up (uses CEDT-Assignment-style)
│   ├── homework-01.pdf          # Compiled/exported assignment
│   ├── code/
│   │   └── homework-01.ipynb    # Jupyter notebook (circuits, code)
│   └── images/                  # Diagrams / figures (if any)
├── homework-02/ ...             # Later homeworks, same layout (notebook and/or LaTeX)
...
- **`homework-xx/`** → Each homework, as a Jupyter notebook and/or LaTeX source, with its compiled PDF and (for LaTeX ones) a per-folder `.latexmkrc`.
...
For notebook-based homeworks (e.g. simulations of consensus/replication protocols), you'll need:

- **Python 3** with `jupyter`, plus whatever libraries the homework calls for — see `.venv/` for the local environment used here.
...
Each LaTeX homework folder has its own `.latexmkrc`, so builds are per-project. Use the root `Makefile`:

```bash
make build DIR=homework-06   # compile once
make watch DIR=homework-06   # rebuild automatically on save (latexmk -pvc)
make clean DIR=homework-06   # remove build artifacts for one project
make clean-all                 # remove build artifacts for every project
```

Or run `latexmk` directly from inside a homework folder:
...
The compiled PDF will appear as `homework-06.pdf` next to the source.
...
For notebook-based homeworks, `make nb-pdf` exports a `.ipynb` straight to PDF via `jupyter nbconvert`, styled with the same `CEDT-Assignment-style.sty` fonts/margins/header as the LaTeX write-ups (via a custom nbconvert template at `vendor/template/templates/nbconvert/cedt-assignment/`):

```bash
make nb-pdf NB=homework-01/code/homework-01-code.ipynb
```
...
jupyter nbconvert --to pdf \
  ...
  homework-01/code/homework-01-code.ipynb
```
...
```bash
make fmt DIR=homework-06   # format one project in place
...
```bash
make new NAME=homework-07
```

This scaffolds `homework-07/` from the shared `assignment-template.tex`, pre-fills the homework number, and copies the shared `.latexmkrc`. The subject line and author block come from `course-config.tex` automatically. Fill in the week number and section/problem content, then build with `make build DIR=homework-07`.

---

## 🩹 Fixing Mistakes

If a mistake turns up in an already-submitted homework — a wrong answer, a typo, a broken calculation — fix it in its own dedicated pull request, scoped to that homework only. Don't bundle it with unrelated tooling or template changes.

- One mistake, one PR, e.g. `gh pr create --title "Homework 05 | Fix wrong answer"`.
- Follow the existing commit style: `<Homework/Scope> | <what changed>`.
...
* Course: (1/2026) *2110473 Fault Tolerant Computing* for CU CEDT
---
